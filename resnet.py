...
class CA(nn.Module):
    def __init__(self, in_channels):
        ...
        self.GR_attention = self.build_GR_attention(in_channels)
        self.spatial_attention = self.build_spatial_attention(in_channels)

    def build_GR_attention(self, in_channels):
    ...
    def build_spatial_attention(self, in_channels):
        return nn.Sequential(
    ...
    def forward(self, x):
        GR_attention = self.GR_attention(x)
        spatial_attention = self.spatial_attention(x)
        x = x * GR_attention + x * spatial_attention
        return x

class BasicBlock(nn.Module):
    expansion: int = 1  # 输出通道数目
...
class ResNet(nn.Module):
    ...
    '''
    def feature_jitter(self, out, device, scale=0.03, prob=0.5):
    ...
    '''
    def feature_jitter(self, out, device, scale=0.003, prob=0.5):
        if random.uniform(0, 1) <= prob:
            N, C, H, W = out.shape
            feat_norm = out.norm(p=2, dim=1).unsqueeze(dim=1) / C  ### p=2  2范数
            jitter = torch.randn((N, C, H, W)).to(device)
            jitter = jitter * feat_norm * scale
            out = out + jitter
            # se1 = SELayer(2048, reduction_ratio=8).to(device)
            # out = se1(out)
        return out.contiguous()
    def _forward_impl(self, x: Tensor) -> Tensor:
        # See note [TorchScript super()]
        npm_a = NPM_torch.NPM(256, 1, 1, 8).to(device)
        npm_b = NPM_torch.NPM(512, 1, 1, 8).to(device)
        npm_c = NPM_torch.NPM(1024, 1, 1, 8).to(device)
        ...
        feature_1 = npm_a(feature_a)

        feature_2 = npm_b(feature_b)

        feature_3 = npm_c(feature_c)
...
class AttnBottleneck(nn.Module):
    ...
    def __init__(
        ...
        planes: int,
        stride: int = 1,
        downsample: Optional[nn.Module] = None,  # 下采样
        groups: int = 1,
        base_width: int = 64,
        dilation: int = 1,
        norm_layer: Optional[Callable[..., nn.Module]] = None,  # 标准化用的层，默认BatchNorm2d
...
class BN_layer(nn.Module):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        # See note [TorchScript super()]
        #x = self.cbam(x)
        #trans = VIT_T.Transformer(2048, 2, 4, 512, 1024, 0.1).to(device)
        ...
        # output = self.se_feature_jitter(output, device, scale=0.001, prob=0.05)
        # print(output.size())
        # output = trans(output)
...
def wide_resnet50_2(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet #pretrained参数为是否使用在ImageNet上的预训练权重
        progress (bool): If True, displays a progress bar of the download to stderr  #progress为进度条可有可无。
    """
